...
class MeterReader:
    ...
    def seg_predict(self, segmenter, imgs, batch_size,img):
        """分割模型完成预测
        ...
        for i in range(0, num_imgs, batch_size):
            ...
            for g in range(len(result)):
                ...
                tn = np.concatenate([t0*1,t0*1,t0*1],axis=2)
                for k in range(t.shape[0]):
                    for j in range(t.shape[1]):
                        ...
                        if(tn[k][j][0]==2):
                            ...
                            im[k][j][1]=0
                            im[k][j][2]=0
                cv2.imwrite("./"+str(self.modelname)+"segresult"+str(g)+".jpg",im)
    ...
    def turn(self,imgs):#图像校正模块
        orig_image = cv2.imread(r'./mould.png', 3)
        skewed_image = imgs
        print("i'm here!!!!!!!!!!!!!!!")
...
# store all the good matches as per Lowe's ratio test.
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            cv2.imwrite('./result/turn_result.jpg',im_out)
            return im_out
    ...
    def predict(self,
        ...
        for i in range(len(sub_imgs)):
            cv2.imwrite("./output/detectresult/"+str(i)+".jpg",sub_imgs[i])
        #change1
        woc=[]  
        img=self.turn(imgs)
        woc.append(img)
        seg_results = self.seg_predict(self.segmenter, woc,seg_batch_size,img)

        #seg_results = self.seg_predict(self.segmenter, sub_imgs,seg_batch_size,img)
        #image_data = np.asarray(seg_results)
        #cv2.imwrite("seg_result.jpg", seg_results[0])
