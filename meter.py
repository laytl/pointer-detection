...
class mential():
    ...
    def ds_ofpoint(self, a, b):#计算两点之间距离
        x1, y1 = a
        x2, y2 = b
...
def linecontours(cp_info):
    ...
    # ************************
    cv2.imwrite('dds.jpg', binary)
    cv2.namedWindow("binary", 0)
    cv2.resizeWindow("binary", 300, 300)  # 设置窗口大小
    ...
    for xx in contours:
        ...
        #print(rect)
        a, b, c = rect#a是旋转矩形的中心（x，y）     b是旋转矩形的宽高（w，h）        c表示倾斜角度
        w, h = b
        w = int(w)
        h = int(h)
        ''' 满足条件:“长宽比例”，“面积”'''
...
def needle(path,img, r, cx, cy,x0,y0):
    ...
    lines = cv2.HoughLinesP(mask, 1, np.pi / 180, 100, minLineLength=int(r / 4), maxLineGap=2)
